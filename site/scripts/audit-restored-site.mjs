...
async function required(relativePath) {
  try {
    ...
    errors.push(`${relativePath}: stránka se nevygenerovala`);
    return '';
  }
}
...
if (!home.includes('/media/generated/prirodni-lekarna/bylinkova-herna-photo.webp')) errors.push('index.html: karta Bylinkové herny nepoužívá fotografický WebP obrázek');
...
const $tinctures = cheerio.load(tinctures);
const tinctureCards = $tinctures('.heritage-directory__card');
if (tinctureCards.length !== 9) errors.push(`Tinktury: očekáváno 9 hlavních karet, nalezeno ${tinctureCards.length}`);
const tinctureImages = tinctureCards.find('.heritage-directory__image img').map((_, element) => $tinctures(element).attr('src') || '').get();
if (tinctureImages.some((src) => !src.endsWith('.webp'))) errors.push(`Tinktury: hlavní karty musí používat WebP obrázky, nalezeno ${JSON.stringify(tinctureImages)}`);
if (tinctureImages.some((src) => src.includes('/obrazky/clanky/tinktury--'))) errors.push('Tinktury: na hlavním rozcestníku zůstal generický SVG obrázek');
if (new Set(tinctureImages).size < 8) errors.push(`Tinktury: očekáváno alespoň 8 různých fotografických podkladů, nalezeno ${new Set(tinctureImages).size}`);
const tinctureHero = $tinctures('.heritage-hub-hero > img').attr('src') || '';
if (!tinctureHero.endsWith('.webp')) errors.push(`Tinktury: hlavní hero obrázek není WebP (${tinctureHero})`);

const tinctureCategorySlugs = [
  'dychaci-cesty-a-nachlazeni',
  'tinktury-imunita-dychani',
  'tinktury-traveni-metabolismus',
  'tinktury-spanek-nervy',
  'tinktury-srdce-krevni-obeh',
  'tinktury-klouby-svaly',
  'tinktury-mocove-cesty-ledviny',
  'tinktury-zeny-muzi',
  'tinktury-detoxikace-ocista',
];
for (const slug of tinctureCategorySlugs) {
  const relative = `tinktury/${slug}/index.html`;
  const html = await required(relative);
  if (!html) continue;
  const $ = cheerio.load(html);
  const hero = $('.heritage-hub-hero > img').attr('src') || '';
  if (!hero.endsWith('.webp')) errors.push(`${relative}: hero kategorie tinktur není WebP (${hero})`);
}
...
for (const file of allFiles.filter((target) => target.endsWith('.html'))) {
  ...
  const critical = $('.critical-recipe-notice').length > 0;
  if (critical) stats.criticalSafetyPages += 1;
...
console.log(`Restoration audit: health cards=${healthCards}, recipe cards=${recipeCards}, tincture cards=${tinctureCards.length}, articles=${stats.articlePages}, critical safety pages=${stats.criticalSafetyPages}, articles with extra ads=${stats.longArticles}, very long=${stats.veryLongArticles}, inline ads=${stats.inlineAdModules}, duplicate heroes=${stats.duplicateHeroImages}, repeated body images=${stats.repeatedBodyImages}.`);
if (errors.length) {
  console.error('Restoration audit failed:');
  errors.slice(0, 160).forEach((error) => console.error(`- ${error}`));
  process.exitCode = 1;
} else {
  console.log('Restoration audit passed: původní rozcestníky, fotografické karty tinktur, obrazová karta herny, reklamy a obrázky článků jsou v pořádku.');
}
