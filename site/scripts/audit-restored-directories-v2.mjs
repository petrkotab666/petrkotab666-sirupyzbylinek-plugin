...
function directoryCards($) {
  return $('.illustrated-directory-card, .heritage-directory__card');
}

function cardHasVisual(card) {
  const imageSrc = card.find('.illustrated-directory-art img, .heritage-directory__image img').first().attr('src') || '';
  if (imageSrc) return true;
  return card.find('.illustrated-directory-art .clean-botanical-art, .heritage-directory__image .clean-botanical-art').length > 0;
}
...
for (const [relative, requirement] of Object.entries(restoredHubs)) {
  ...
  if (!$('.heritage-hub-hero__copy, .visual-section-hero-copy').length) errors.push(`${relative}: chybí jednotná centrovaná hlavička rozcestníku`);
  if (!$('.heritage-directory .section-heading.centered-heading, .restored-hub-section .section-heading.centered-heading').length) errors.push(`${relative}: chybí centrovaný nadpis obrazového rozcestníku`);
  ...
  cards.each((index, node) => {
    ...
    if (!cardHasVisual(card)) errors.push(`${relative}: karta ${index + 1} nemá obrazovou grafiku`);
    if (card.find('.clean-botanical-art').length) generatedCardVisuals += 1;
...
if (home) {
  ...
  if (gameImage !== '/media/generated/prirodni-lekarna/bylinkova-herna-photo.webp') errors.push(`index.html: Bylinková herna nepoužívá sjednocený WebP obrázek, nalezeno ${JSON.stringify(gameImage)}`);
}
...
await writeFile(path.join(DIST, 'restored-directories-audit.md'), [
  ...
  `- Karet s generovanou botanickou grafikou: ${generatedCardVisuals}`,
...
console.log(`Restoration audit v2: ${Object.keys(restoredHubs).length} hub pages, ${restoredCardCount} cards (${generatedCardVisuals} generated visuals), ${articlePages} articles, ${monetizedArticles} monetized, ${duplicateHeroImages} duplicate hero images, ${errors.length} errors.`);
if (errors.length) {
