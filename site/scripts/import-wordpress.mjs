...
const USER_AGENT = 'SirupyZBylinek migration bot/1.0 (+https://www.sirupyzbylinek.cz/)';

const skipPrefixes = ['/wp-admin', '/wp-login', '/wp-json', '/xmlrpc', '/feed', '/comments', '/author/', '/tag/', '/category/', '/search/'];
...
async function fetchText(url, timeoutMs = 25000) {
  ...
  const promise = (async () => {
    ...
    try {
      ...
      return { text: await response.text(), contentType: response.headers.get('content-type') || '', finalUrl: response.url };
    } finally {
      clearTimeout(timer);
    }
  })();
...
async function discoverSitemapUrls() {
  ...
  try {
    ...
    errors.push({ url: new URL('/robots.txt', BASE).href, error: String(error) });
  }
  ...
  async function readMap(url) {
    ...
    try {
      ...
      const locs = [...result.text.matchAll(/<loc>([\s\S]*?)<\/loc>/gi)].map((m) => decodeXml(m[1].trim()));
  ...
  while (queue.length && visited.size < MAX_PAGES) {
    ...
    try {
      ...
      $('a[href]').each((_, element) => {
        ...
        if (next && !visited.has(next) && !queue.includes(next) && visited.size + queue.length < MAX_PAGES * 1.5) queue.push(next);
      });
    } catch (error) {
      errors.push({ url, error: String(error) });
    }
...
function safeSegment(value) {
  return decodeURIComponent(value).normalize('NFKD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^a-z0-9._-]+/g, '-').replace(/^-+|-+$/g, '') || 'stranka';
}
...
function extensionFrom(contentType, sourceUrl) {
  ...
  if (map[contentType?.split(';')[0]?.toLowerCase()]) return map[contentType.split(';')[0].toLowerCase()];
...
async function downloadImage(source, pageKey) {
  ...
  let absolute;
  try { absolute = new URL(source, BASE).href; } catch { return source; }
  if (downloadedImages.has(absolute)) return downloadedImages.get(absolute);
  try {
    const response = await fetch(absolute, { headers: { 'user-agent': USER_AGENT, accept: 'image/*' }, redirect: 'follow' });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    ...
    const original = safeSegment(path.basename(new URL(response.url).pathname, path.extname(new URL(response.url).pathname)));
    ...
    errors.push({ url: absolute, error: `image: ${String(error)}` });
    return absolute;
  }
}
...
async function extractPage(url) {
  ...
  const selector = ['main article .entry-content', 'article .entry-content', '.elementor-widget-theme-post-content', 'main .page-content', 'main article', 'main'].find((candidate) => $(candidate).first().text().trim().length > 120);
  if (!selector) return null;
  const content = $(selector).first().clone();
  content.find('script,style,noscript,iframe,form,button,svg,nav,footer,header,.comments-area,.post-navigation,.sharedaddy,.jp-relatedposts,.cookie,.cky-consent-container,.cmplz-cookiebanner,[class*="cookie"],[id*="cookie"],[class*="consent"],[id*="consent"],.sidebar,.widget-area').remove();
  content.find('h1').first().remove();

  const title = cleanText($('h1').first().text() || $('meta[property="og:title"]').attr('content') || $('title').text().split('|')[0]);
  if (!title || title.length < 3) return null;
  const firstParagraph = cleanText(content.find('p').filter((_, el) => cleanText($(el).text()).length > 60).first().text());
  const description = cleanText($('meta[name="description"]').attr('content') || $('meta[property="og:description"]').attr('content') || firstParagraph).slice(0, 320);
  const published = dateOnly($('meta[property="article:published_time"]').attr('content') || $('time[datetime]').first().attr('datetime'));
  const updated = dateOnly($('meta[property="article:modified_time"]').attr('content'));
  const category = cleanText($('meta[property="article:section"]').attr('content') || $('.cat-links a,.category a').first().text() || 'Bylinky');
  ...
  const images = content.find('img').toArray().slice(0, 20);
  for (const element of images) {
    ...
    $(element).attr('src', local).removeAttr('srcset data-src data-lazy-src sizes loading');
  }
  ...
  content.find('a[href]').each((_, element) => {
    ...
    try {
      ...
      if (target.hostname.replace(/^www\./, '') === BASE.hostname.replace(/^www\./, '')) $(element).attr('href', target.pathname + target.search + target.hash);
    } catch {}
  });

  const turndown = new TurndownService({ headingStyle: 'atx', bulletListMarker: '-', codeBlockStyle: 'fenced', emDelimiter: '*', strongDelimiter: '**' });
  turndown.addRule('removeEmptyLinks', { filter: (node) => node.nodeName === 'A' && !node.textContent.trim() && !node.querySelector('img'), replacement: () => '' });
  const markdown = turndown.turndown(content.html() || '').replace(/\n{3,}/g, '\n\n').trim();
  ...
  return { pathname, file: contentFileFor(pathname), body: `${frontmatter}\n\n${markdown}\n`, title };
}
...
async function main() {
  ...
  for (const url of urls) {
    ...
    try {
      ...
      pages.push({ url, path: page.pathname, file: path.relative(ROOT, page.file), title: page.title });
    } catch (error) {
      errors.push({ url, error: String(error) });
    }
  }
  const report = { generatedAt: new Date().toISOString(), baseUrl: BASE.href, sitemapUrls: sitemapUrls.size, crawledUrls: urls.size, importedPages: pages.length, downloadedImages: downloadedImages.size, pages, errors };
  await writeFile(REPORT_FILE, JSON.stringify(report, null, 2) + '\n');
  console.log(`Imported ${pages.length} pages and ${downloadedImages.size} images.`);
  if (pages.length < 5) throw new Error('Import found fewer than five usable pages. Review site access and selectors.');
}
